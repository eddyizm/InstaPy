...
def unfollow():
    try:
        ...
        session.set_do_comment(True, percentage=20)
        session.set_comments([u':clap:', u':thumbsup:', u':raised_hands:'])
        session.unfollow_users(amount=2, onlyInstapyFollowed = True, onlyInstapyMethod = 'FIFO', sleep_delay=60)
...
schedule.every().saturday.at("22:30").do(interact)  
schedule.every().sunday.at("22:30").do(quickstart)  
